...
class GraphfromString:
    ...
    def create(self, InpString):  #Create the Graph from a String input
    ...
    def GetDistanceofRoute(self, RouteStr):  #Calculate the distance of a given route ("A-B-C-D" format)
        list_nodes = RouteStr.split("-")
        dist=0
        for i in range(len(list_nodes)-1):
            if self.Graph.istherearoute(list_nodes[i], list_nodes[i+1]):
    ...
    def BreadthFirstTraversal(self, From, StepLimit, DistLimit):
        list_nodes=[StepLimit]
        list_nodes[0]={From : [0]}  #In 0 step we can reach only the starting point (distance=0)
        step=0;
    ...
    def GetNumofTripsMaxStep(self, A, B, MaxStep):  #Calculate the number of trips from A to B with maximum MaxStep steps
    ...
    """To calculate the shortest route I'm using here again 
       ...
       The StepLimite is the number of edges (this is the deepest 
    ...
    def GetNumofTripsMaxDist(self, A, B, MaxDist):  #Calculate the number of trips from A to B with maximum MaxDist distance
