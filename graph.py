class GraphClass:
    ...
    def addroute(self, A, B, distance): #Add a new route between two nodes
    ...
    def istherearoute(self, A, B): #Check if route exists between A and B
        if B in self.graph[A]:
            return True
        else:
            return False

    def getdistance(self, A, B): #Get the distance between A and B
        if self.istherearoute(A, B):
            return int(self.graph[A][B])
        else:
            return -1 #If no route between A and B, return -1

    def getroutesfrom(self, A): #Get the list of directly accessible nodes from A
        if A in self.graph:
            return self.graph[A]
